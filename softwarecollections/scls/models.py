...
ICON_NAMES = sorted(
    tuple(
        ...
        for name in os.listdir(
            os.path.join(os.path.dirname(__file__), "static", "scls", "icons")
        )
...
POLICY_TEXT = {
    ...
    "is cared for, but the developers make no commitments to update "
    "the repositories in a timely manner. ",
...
POLICY_CHOICES_TEXT = [
    (key, mark_safe(markdown2.markdown(POLICY_TEXT[key]))) for key in POLICIES
]
...
class Copr(models.Model):
    ...
    def last_modified(self):
        return (
            self.detail["last_modified"]
            and datetime.utcfromtimestamp(self.detail["last_modified"]).replace(
                tzinfo=utc
            )
            or None
        )
...
class OtherRepo(models.Model):
    ...
    version = models.CharField(
        _("Distribution version"), max_length=20, blank=True, default=""
    )
    ...
    icon = models.CharField(
        _("Icon"), max_length=20, choices=ICON_NAME_CHOICES, blank=True, default=""
    )
    ...
    def __str__(self):
        return "{name} {version} {variant} {arch}".format(
            name=self.name, version=self.version, variant=self.variant, arch=self.arch
        )
...
class SoftwareCollection(models.Model):
    ...
    other_repos = models.ManyToManyField(
        OtherRepo, verbose_name=_("Other repos"), blank=True
    )
    upstream_url = models.URLField(_("Project homepage"), blank=True)
    issue_tracker = models.URLField(
        _("Issue Tracker"),
        blank=True,
    ...
    collaborators = models.ManyToManyField(
        ...
        related_name="softwarecollection_set",
        blank=True,
    )
    requires = models.ManyToManyField(
        "self", symmetrical=False, related_name="required_by", editable=False
    )
        ...
        return ", ".join(
            [
                '<a href="{url}" target="_blank">{username}/{name}</a>'.format(
                    url=copr.get_url(), username=copr.username, name=copr.name
                )
    ...
    def get_title_tag(self):
        return '<a href="{url}" target="_blank">{title}</a>'.format(
            url=self.get_absolute_url(), title=self.title
        )
...
""".format(
            name=self.name, maintainer=self.maintainer.get_username(), slug=self.slug
        )
            ...
            for copr in self.all_coprs:
                ...
                for repo in self.repos.filter(copr=copr):
                    if repo.name not in copr.yum_repos:
                        ...
                        repos_config.append(
                            "[{name}]\nname={name}\nbaseurl={url}\ngpgcheck=0\n".format(
                                name=repo.name, url=repo.copr_url
                            )
                        )
    ...
    def check_repos_content(self, timeout):
        with self.lock:
            ...
            for repo in self.repos.all():
                ...
                repo.save()
            self.has_content = (
                len(self.dump_provides(timeout)) + self.other_repos.count()
            ) > 0
            self.save()
        ...
        with self.lock:
            out = check_output(
                (
                    ...
                    "done | sed -r -e 's|^{all_repos_root}/||' -e 's|/.provides$||' | sort -u".format(
                        all_repos_root=settings.REPOS_ROOT,
                        repos_root=self.get_repos_root(),
                    )
                ),
                shell=True,
...
class Repo(models.Model):
    ...
    scl = models.ForeignKey(
        SoftwareCollection, related_name="repos", on_delete=models.CASCADE
    )
    ...
    @property
    def rpmname(self):
        return "-".join([self.scl.maintainer.get_username(), self.scl.name, self.name])

    @property
    def rpmfile(self):
        return "-".join([self.rpmname, VERSION, RELEASE]) + ".noarch.rpm"
    ...
    def get_rpmfile_path(self):
        return os.path.join(
            self.scl.get_repos_root(), self.name, "noarch", self.rpmfile
        )

    def get_rpmfile_symlink_path(self):
        return os.path.join(
            self.scl.get_repos_root(), self.name, "noarch", self.rpmfile_symlink
        )
    ...
    def get_download_url(self):
        return reverse(
            "scls:download", kwargs={"slug": self.slug}
        ) + self.rpmfile_symlink

    def get_oses_names_and_logos(self):
        if self.distro == "epel":
            return [
                ("RHEL {}".format(self.version), get_icon_url("rhel")),
    ...
    def rpmbuild(self, timeout=None):
        with self.lock:
            ...
            defines = [
                "-D",
                "_topdir {}".format(settings.RPMBUILD_TOPDIR),
                "-D",
                "_rpmdir {}".format(self.get_repo_dir()),
                "-D",
                "dist {}".format(self.distro_version),
                "-D",
                "scl_name {}".format(self.scl.name),
                "-D",
                "scl_title {}".format(self.scl.title),
                "-D",
                "scl_description {}".format(self.scl.description.replace("%", "%%")),
                "-D",
                "repo_name {}".format(self.name),
                "-D",
                "pkg_name {}".format(self.rpmname),
                "-D",
                "pkg_version {}".format(VERSION),
                "-D",
                "pkg_release {}".format(RELEASE),
                "-D",
                "repo_distro {}".format(self.distro),
                "-D",
                "repo_arch {}".format(self.arch),
                "-D",
                "repo_baseurl {}".format(self.get_repo_url()),
            ]
...
class Score(models.Model):
    scl = models.ForeignKey(
        SoftwareCollection, related_name="scores", on_delete=models.CASCADE
    )
