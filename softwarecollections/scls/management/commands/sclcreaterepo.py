...
class Command(LoggingBaseCommand):
    ...
    args = "[ <repo_slug> ... ]"
    help = "Rebuild metadata for all repos. " "Optionaly you may specify one or more slug of particular repo to be processed."
    ...
    def handle(self, *args, **options):
        ...
        with Pool(processes=int(options["max_procs"])) as pool:
            errors += sum(pool.map(createrepo, [(scl, timeout) for scl in repos]))
            if errors > 0:
                raise CommandError(
