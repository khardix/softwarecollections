...
class Command(LoggingBaseCommand):
    ...
    args = "[ <scl_slug> ... ]"
    help = "Dump provides for all collections. " "Optionaly you may specify one or more slug of particular SCLs to be dumped."
    ...
    def handle(self, *args, **options):
        ...
        with Pool(processes=int(options["max_procs"])) as pool:
            errors += sum(pool.map(dump_provides, [(scl, timeout) for scl in scls]))
            if errors > 0:
                raise CommandError(
                    "Failed to dump provides: {} error(s)".format(errors)
                )
